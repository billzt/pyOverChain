...
from pychain.core import axtchain, blat, chainMergeSort, chainNet, faSplit, liftUp, netChainSubset

def run(args:dict):
    logfile = 'pyChainTmp.log/'+args['file_old']\
        +args['chr_old']+'-to-'+args['file_new']+args['chr_new']+'.log'

    flog = open(logfile, mode='w')

    print(f'[pychain:{datetime.datetime.now()}] Begin faSplit...', file=flog, flush=True)
    faSplit.run(file_new=args['file_new'], chr_new=args['chr_new'], logfile=logfile)

    print(f'[pychain:{datetime.datetime.now()}] Begin BLAT...', file=flog, flush=True)
    ...
    print(f'[pychain:{datetime.datetime.now()}] Begin liftUp...', file=flog, flush=True)
    liftUp.run(file_new=args['file_new'], chr_new=args['chr_new'], logfile=logfile)

    print(f'[pychain:{datetime.datetime.now()}] Begin axtchain...', file=flog, flush=True)
    axtchain.run(file_old=args['file_old'], file_new=args['file_new'], \
        chr_old=args['chr_old'], chr_new=args['chr_new'], logfile=logfile)
    
    print(f'[pychain:{datetime.datetime.now()}] Begin chainMergeSort...', file=flog, flush=True)
    chainMergeSort.run(file_new=args['file_new'], chr_new=args['chr_new'], logfile=logfile)

    print(f'[pychain:{datetime.datetime.now()}] Begin chainNet...', file=flog, flush=True)
    chainNet.run(file_old=args['file_old'], file_new=args['file_new'], \
        chr_new=args['chr_new'], logfile=logfile)

    print(f'[pychain:{datetime.datetime.now()}] Begin netChainSubset...', file=flog, flush=True)
